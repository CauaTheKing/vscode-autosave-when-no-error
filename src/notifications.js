...
const messages = ["You just saved {MINUTES} {MINUTES_LABEL} by using VSCode AutoSave.\n\nCongrats!", "Good news!\n\nVSCode AutoSave saved you {MINUTES} {MINUTES_LABEL}.", "Congrats!\n\nYou've saved {MINUTES} {MINUTES_LABEL} by using VSCode AutoSave.\n\nKeep up the good work!", "You saved {MINUTES} {MINUTES_LABEL} by using VSCode AutoSave.\n\nThat's pretty amazing, right?"];

const messageMinute = (minutes) => {
    const label = minutes > 1 ? "minutes" : "minute";
    const message = messages[Math.floor(Math.random() * messages.length)];
    return message.replace("{MINUTES}", minutes).replace("{MINUTES_LABEL}", label);
};
...
const actions = {
    "Add a Review": async ({ context, minute }) => {
        vscode.env.openExternal(vscode.Uri.parse("https://marketplace.visualstudio.com/items?itemName=codista.vscode-autosave&ssr=false#review-details", true));
        track(
            context,
    ...
    "Disable Progress Notifications": async ({ context, minute }) => {
        ...
        await configs.update("disableProgressNotifications", true, vscode.ConfigurationTarget.Global);
        track(context, TRACKED_ACTIONS.SILENCED_PROGRESS, { at: new Date().toUTCString() }, { minute });
    }
};

const notifyProgressAndUpdateHistory = async ({ read, write, context }, min) => {
...
const notifyProgressIfNeeded = async (context, { read, write }, saveCount) => {
    const disabledNotifications = vscode.workspace.getConfiguration("cau-vscode-autosave").get("disableProgressNotifications");

    if (disabledNotifications) return;

    const [lastMinuteNotified] = (read(stateKeys.MINUTES_NOTIFIED) || []).slice(-1);

    const passedMinutes = minutesWithCountToNotify.filter((x) => saveCount >= x.saveCount);
    ...
    if (!lastMinuteNotified || nextMinuteToNotify > lastMinuteNotified) {
        await notifyProgressAndUpdateHistory({ context, read, write }, nextMinuteToNotify);
    }
};
